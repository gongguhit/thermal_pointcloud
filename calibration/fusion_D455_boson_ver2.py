...
## read the datas ##
#RGBD data
rgbIntrinsic = np.load("/home/gg/Downloads/calibration/calib_data/boson_realsense_RGB_intrinsic.npy")
rgbDistortion = np.load("/home/gg/Downloads/calibration/calib_data/boson_realsense_RGB_distortion.npy")

#thermal data
thermalIntrinsic = np.load("/home/gg/Downloads/calibration/calib_data/boson_realsense_Thermal_intrinsic.npy")
thermalDistortion = np.load("/home/gg/Downloads/calibration/calib_data/boson_realsense_Thermal_distortion.npy")

# rotation and translation data
relative_R = np.load("/home/gg/Downloads/calibration/calib_data/boson_realsense_Ralative_rotation_matrix.npy")
relative_T = np.load("/home/gg/Downloads/calibration/calib_data/boson_realsense_Ralative_translation_matrix.npy")
...
rgbImage = cv2.imread('/home/gg/Downloads/calibration/calib_data/RGB_boson_img/raw/RealSense_RGB0.png')
rgbImage_g = cv2.cvtColor(rgbImage, cv2.COLOR_BGR2GRAY)   #graylize
thermalImage = cv2.imread('/home/gg/Downloads/calibration/calib_data/boson_img/raw/boson0.png')
...
def get_rgbd_stream(): # with depth and color aligned
    ...
    for i in range(len(vtx)):
        pcdata = [np.float(vtx[i][0])*1000,np.float(vtx[i][1])*1000,np.float(vtx[i][2])*1000,np.float(colorful[i][0]),np.float(colorful[i][1]),np.float(colorful[i][2])]
...
if __name__ == "__main__" :
    ...
    while True :
        ...
        thermal_color_resized = cv2.applyColorMap(cv2.resize(thermalImage[:,:], (368, 276)),cv2.COLORMAP_JET)

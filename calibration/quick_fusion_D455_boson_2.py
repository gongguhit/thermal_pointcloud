...
## read the datas ##
#RGBD data
rgbIntrinsic = np.load("/home/gg/Downloads/calibration/test_data_2/boson_realsense_RGB_intrinsic.npy")
rgbDistortion = np.load("/home/gg/Downloads/calibration/test_data_2/boson_realsense_RGB_distortion.npy")

#thermal data
thermalIntrinsic = np.load("/home/gg/Downloads/calibration/test_data_2/boson_realsense_Thermal_intrinsic.npy")
thermalDistortion = np.load("/home/gg/Downloads/calibration/test_data_2/boson_realsense_Thermal_distortion.npy")

# rotation and translation data
relative_R = np.load("/home/gg/Downloads/calibration/test_data_2/boson_realsense_Ralative_rotation_matrix.npy")
relative_T = np.load("/home/gg/Downloads/calibration/test_data_2/boson_realsense_Ralative_translation_matrix.npy")
...
def get_thermal_stream():
    ...
    return thermal_img_raw,thermal_img
...
if __name__ == "__main__" :
    ...
    # counter = 0
    while True :
        thermal_img_raw,thermal_img = get_thermal_stream()
        # counter += 1
        thermal_img_gray = ((thermal_img_raw-27313)*0.04).astype(np.uint8)
        thermalImage = cv2.resize(thermal_img_gray[:,:], (640, 480))
        data = cv2.resize(thermal_img_raw[:,:], (640, 480))
        ...
        # minVal, maxVal, minLoc, maxLoc = cv2.minMaxLoc(thermalImage)
        ...
        # thermal_color_resized = cv2.applyColorMap(cv2.resize(thermal_inverse[:,:], (368, 276)),cv2.COLORMAP_JET)
        thermal_img = cv2.resize(thermal_img[:,:],(368,276))
        ...
        imgs = np.hstack([RGBImage_cut,depth_colormap_cut,thermal_img])
        cv2.imshow("input_images",imgs)
        # if (time.time()-start_time)!=0:
            # cv2.namedWindow('thermal image',0)
            # cv2.resizeWindow('thermal image',640,480)
            # cv2.putText(thermal_img,"FPS {0}".format(float('%.1f'%(counter/(time.time() - start_time)))),(200,50),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),3)
            # imgs = np.hstack([RGBImage_cut,depth_colormap_cut,thermal_img])
            # cv2.imshow("input_images",imgs)
            # cv2.imshow("fusion_img_cut",fusion_img_cut_JET)
